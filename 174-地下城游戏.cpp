...
class Solution {
   public:
    int calculateMinimumHP(vector<vector<int>>& dungeon) {
        ...
        vector<vector<int>> dp(row + 1, vector<int>(col + 1, 1 << 30));
        dp[row][col - 1] = dp[row - 1][col] = 1;  // 走出终点后起码还要有一点血
        for (int i = row - 1; i >= 0; --i) {
            for (int j = col - 1; j >= 0; --j) {
                // 我已经在i，j上了，所以我要选到终点消耗血量最少的下一点
                int minHp = min(dp[i + 1][j], dp[i][j + 1]);
                // 我在这一点需要的血量是上一步选的下一点需要的血量加上这一点要被减去的血量，由于扣血是负，所以加上该值使用减法
                // 我要保证有一点血
...
int main(void) {
    ...
    auto rel = solution->calculateMinimumHP(
        *(new vector<vector<int>>{{-2, -3, 3}, {-5, -10, 1}, {10, 30, -5}}));
    cout << rel << endl;
