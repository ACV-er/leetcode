...
class Solution {
   public:
    ...
    int romanToInt(string s) {
        int curIdx = 0, rel = 0, add = 0;
        while (curIdx < s.length() - 1) {
            add = roman_to_int_map[s.substr(curIdx, 2)];
            if (add == 0) {
                rel += roman_to_int_map[s.substr(curIdx, 1)];
            } else {
                rel += add;
                curIdx++;
            }
...
int main(void) {
    ...
    cout << solution->romanToInt("III") << "=3" << endl;
    cout << solution->romanToInt("LVIII") << "=58" << endl;
    return 0;
}
